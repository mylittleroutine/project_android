...
class AdapterAppInfoList(val context: Context, var list: List<ApplicationInfo>): RecyclerView.Adapter<AppInfoListHolder>() {
    var items = list
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AppInfoListHolder {
        ...
        return AppInfoListHolder(context, bind)
    }
...
class AppInfoListHolder(val context: Context, var bind:ItemAppInfoBinding): RecyclerView.ViewHolder(bind.root) {
    fun setItem(item:ApplicationInfo){
        ...
        var notUseAppList = AppUtil.loadNotUseAppList(context)
        ...
        bind.checkBox.setOnCheckedChangeListener{ _, isChecked ->
            if(isChecked){
                AppUtil.addNotUseApp(context, item)
            }
            else{
                AppUtil.deleteNotUseApp(context, item)
            }
